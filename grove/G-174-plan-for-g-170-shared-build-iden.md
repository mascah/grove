...
- **Release inputs.** Two unexported variables set by the linker:
  ...
  `-X github.com/mascah/grove.revision=<40-hex commit>`. A stamped value wins
  over Go's build information; without one, the module version
  (`(devel)`, a `go install` tag or pseudo-version) and `vcs.revision` /
  `vcs.modified` remain the fallback. A missing revision prints
  `revision unknown` rather than nothing, so an unstamped archive build never
...
2. `identity.go` with a pure `identify(info, ok, version, revision)` and
   table tests: release stamped, stamped without revision, development with
