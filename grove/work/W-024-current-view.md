...
4. For the observed W-012 history, an old Proposed copy on W-010 does not
...
5. Preserve read-only behavior, incomplete-result diagnostics, safe action
   targeting and W-013's batched Git reads. Avoid per-branch Git processes;
   any added history work must retain acceptable measured board load behavior.
6. Update Q-001's implementation note and current documentation. Full visual
   redesign is W-025; projection should be independently testable and explainable.
