...
[G-035](G-035-interactive-adoption.md) records that authority
and the policies it revises; [G-064](G-064-stable-knowledge.md)
records the owner's later storage/identity decision. Work records own acceptance; the
[record model](../docs/record-model.md) describes the currently implemented schema.
...
- Stable sequential IDs coordinated across linked worktrees; revision-checked
  ...
  ordinary edits preserve identity. G-052 explicitly reconciles this repo's
...
G-038 must migrate that meaning explicitly and preserve historical evidence.
...
G-065 supplies the support; G-052 reconciles all existing Grove records and
...
board and on-demand card history are integrated. G-023's work guide and local
...
[G-036](G-036-interactive-adoption.md) owns the first adoption
milestone. Its [roadmap](G-047-adoption-roadmap-plan.md) orders independently
assignable work and later investments; the
[evaluation](G-048-direction-evaluation-review.md) retains findings,
research and limits rather than another editable product direction.

Start with shaping ([G-025](G-025-shaping-entrypoint.md)), follow
with knowledge/artifact support, flexible identity/storage (G-065), complete
repository-content reconciliation (G-052), and the review handoff; exercise a real Grove
...
Next owns that, starting from G-036's; change this section only when the
