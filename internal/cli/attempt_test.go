...
func TestAttemptCommandsUsage(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{[]string{"show", "G-001", "--model", "x"}, 2, "--budget, --permission-mode, --model, and --effort apply only to run and resolve"},
		{[]string{"show", "G-001", "--effort", "high"}, 2, "apply only to run and resolve"},
		{[]string{"resolve", "G-001", "--until", "plan"}, 2, "--until, --branch, and --worktree apply only to run"},
		{[]string{"resolve", "G-001", "G-002"}, 2, "resolve requires exactly one record ID"},
		{[]string{"resolve", "G-001", "--budget", "1"}, 1, "resolve needs target: BRANCH in grove.yaml"},
