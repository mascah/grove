...
- `go run ./cmd/grove check`: OK, 29 records. Relative links in the changed
  files resolve (command and result in the record's evidence).
...
See the record's Next for the reviewer's findings and their dispositions.
