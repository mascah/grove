...
- `/grove-work` in a fresh **interactive** Claude Code session on a real
  prepared assignment, and the owner's judgment of whether it replaces asking
  for a prompt. This is what keeps W-010 active. The owner chose
  `/grove-work W-012` from the main checkout as that run.
- `$grove-work` in the interactive Codex TUI, and whether `agents/openai.yaml`
  prevents implicit invocation.
- Interruption of a session midway through implementation, and resume from
  there. Resume was exercised only from a committed checkpoint, in simulation,
  against the first guide.
