...
W-003 updates, W-004 versions, W-005 workspace resolution, their shared
...
- W-003's `14ed115` review fix and `6a5e297` closure are ancestors of main,
  integrated by `b20d2b0`. W-004's `ca420f5` and W-005's `d232aa2`
  review fixes and both closures are ancestors, integrated by `5041ae1`.
- The [update plan](../plans/W-003-update.md) and
  [coordination plan](../plans/W-004-W-005-coordination.md) retain reviewed
...
| Real `versions W-005 --json` → all four selectors → `workspace --json` → `show --project` | All resolved and read the exact selected bytes |
...
projects distinct from invalid/foreign ones. Repair: [W-006](../../grove/work/W-006-workspace-provenance.md).
...
post-check race boundary. Repair: [W-006](../../grove/work/W-006-workspace-provenance.md).
...
The comment precedes the value's syntax span and is protected by W-003's
...
Repair: [W-007](../../grove/work/W-007-preserve-updates.md).
...
that no accepted fixture used them. The actual reader accepts them; W-003 says
...
The configuration bytes changed before the comparison, so W-003 requires refusal.
...
Repair: [W-008](../../grove/work/W-008-git-paths.md).
...
W-003's Next still asked for its already-completed integration; Q-001 said no
combined view was implemented; W-004/W-005 opened with proposed-only wording
despite later implementation evidence. Preserve historical review evidence and
Q-001's accepted answer, but make current Next actions and implementation state
explicit. The record model's W-002 operational link also called creation proposed.
...
serially in order W-006 → W-007 → W-008; the order is execution coordination,
...
refusals remain visible. [W-009](../../grove/work/W-009-terminal-picker.md)
...
This keeps Q-001 intact and avoids building actions on the defects found here.
...
remain unchanged; the retained W-004/W-005 checkout remains clean. The only
