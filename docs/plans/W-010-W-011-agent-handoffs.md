...
**Specs:** [W-010](../../grove/work/W-010-work-handoffs.md) owns work assignments;
[W-011](../../grove/work/W-011-shaping-entrypoint.md) owns authoring instructions.
...
verify W-006–W-008 repair/review commits are integrated. Reuse the delivered
W-008 path helpers; do not copy unfinished branch code. The owner has scheduled
W-009 after repairs. Prefer this implementation after that handoff to avoid
concurrent edits to `internal/cli/cli.go`, README, and the brief. W-009 is not a
semantic prerequisite; if scheduling differs, coordinate ownership explicitly.

Keep W-010 and W-011 proposed until their implementation starts. Retain individual
acceptance/evidence even with a shared plan; no release umbrella or new metadata.
Preserve the existing explicit commands and W-009's default TUI invocation.
...
Claude: /grove-work W-010 W-011
Codex:  $grove-shape <idea or existing work ID>
Codex:  $grove-work W-010 W-011
CLI:    go run ./cmd/grove context W-010 W-011 --json
```
...
## Task 1: Assemble selection, relationships, and provenance (W-010)
...
## Task 2: Add bounded source inclusion and consistent output (W-010)
...
## Task 3: Expose the read-only command (W-010)
...
reject context-only options on other commands, including W-009's default TUI.
...
- [ ] Dispatch Build with `context.Background()` from the ordinary CLI. Preserve
  any context-aware repo APIs delivered by W-009 rather than creating duplicates.
...
## Task 4: Deliver the shared guides and thin adapters (W-010/W-011)
...
- [ ] In disposable repositories, exercise serial W-006–W-008-shaped repair
  selection and a W-009-shaped dependent item: external blockers remain visible,
...
- [ ] Run `go test -count=1 ./...`, `go test -race -count=1 ./...`, `go vet ./...`,
  ...
  With W-009 integrated, verify the new explicit command does not disturb bare
