# W-001 inspection implementation plan

Goal: deliver the read-only Go CLI described by [W-001](../../grove/work/W-001-inspect-records.md).
Spec: [record model](../record-model.md), including its first-reader proposals.
...
- README, AGENTS, model, brief, W-001: invocation instructions and verified state.
...
- [x] Review and verify: run `go test ./...`, `go test -race ./...`, `go vet ./...`,
  ...
  before/after commands. Check doc links and reconcile W-001 and the brief.
...
implementation. Completed 2026-09-19; W-001 is reconciled and closed.

- `go build ./...`, `go vet ./...`, `gofmt -l .` (clean), `go test ./...`, and
  `go test -race ./...` pass for `internal/cli` and `internal/project`.
- `list`, `show W-001`, and `check` run against Grove's four records; `check`
